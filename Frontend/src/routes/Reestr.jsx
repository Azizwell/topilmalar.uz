import {
  setDistrict,
  setHasUser,
  setItem,
  setMarket,
  setReestrData,
  setRegion,
  setSearch,
  setUser,
  setViewReestr,
  setViewReestrItem,
} from "../store/slices/appSlices";
import { useNavigate } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faArrowRightFromBracket } from "@fortawesome/free-solid-svg-icons";
import { useEffect } from "react";
import {
  useGetItemsQuery,
  useGetMarketsQuery,
  useGetMyQueryQuery,
  useGetOneReestrMutation,
  useGetRegionsQuery,
} from "../store/services/globalServices";

const Reestr = () => {
  const {
    user,
    market,
    item,
    search,
    region,
    district,
    reestrData,
    viewReestrItem,
    viewReestr,
  } = useSelector((state) => state.app);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { data: marketData, isSuccess: marketIssucces } = useGetMarketsQuery();
  const { data: itemData, isSuccess: itemIssuccess } = useGetItemsQuery();
  const { data: regionData, isSuccess: regionIssuccess } = useGetRegionsQuery();
  const {
    data: queryData,
    isSuccess: queryIssuccesss,
    isLoading,
    refetch,
  } = useGetMyQueryQuery(search);

  const [getOneReestr, { isSuccess }] = useGetOneReestrMutation();

  function getReestrDetails(item) {
    console.log(item.entityId);
    getOneReestr(item.entityId).then((res) => {
      console.log(res.data);
      if (res.data.market) {
        dispatch(setViewReestr(res.data));
      } else {
        dispatch(setViewReestrItem(res.data));
      }
    });
  }
  useEffect(() => {
    if (viewReestrItem.id || viewReestr.id) {
      navigate("/view_reestr");
    }
  }, [viewReestrItem, viewReestr]);

  useEffect(() => {
    if (queryIssuccesss) {
      dispatch(setReestrData(queryData));
    }
    console.log(reestrData);
    console.log();
  }, [queryIssuccesss]);

  useEffect(() => {
    if (marketIssucces) {
      dispatch(setMarket(marketData));
    }
  }, [marketIssucces]);

  useEffect(() => {
    if (itemIssuccess) {
      dispatch(setItem(itemData));
    }
  }, [itemIssuccess]);

  useEffect(() => {
    if (regionIssuccess) {
      dispatch(setRegion(regionData));
    }
  }, [regionIssuccess]);

  useEffect(() => {
    refetch().then((res) => {
      dispatch(setReestrData(res.data));
    });
  }, [queryData]);

  useEffect(() => {
    if (search.region != "") {
      region.forEach((item) => {
        if (item.id === search.region) {
          dispatch(setDistrict(item.district));
        }
      });
    }

    if (search.region == "") {
      dispatch(setDistrict([]));
    }

    dispatch(setSearch({ ...search, district: "" }));
  }, [search.region]);



  const logOutHandler = () => {
    localStorage.removeItem("user");

    dispatch(setUser(null));
    dispatch(setHasUser(false));
    // navigate("login");
  };

  function apply(params) {
    if (localStorage.getItem("user")) {
      dispatch(setHasUser(true));
      console.log("true");
      navigate("/");
    } else {
      navigate("/login");
    }
  }

  return (
    <>
      <div className="   navbar-sys d-lg-flex d-grid items-center">
        <div className=" p-10 flex justify-between items-center w-100">
          <a className="logo-system text-decoration-none " href="/">
            <img
              src="data:image/png;base64,"
              alt="topilmalar-logo"
            />
          </a>
          <div className="list-page d-none d-lg-flex align-items-center">
            <a href="/reestr">Reyestr</a>
            <a href="/">Statistika</a>
            <a href="/">Tizim haqida</a>
            <a href="/">Repo</a>
          </div>
          <div className=" flex gap-1 items-center  ">
            <button
              onClick={() => apply()}
              className="bg-blue-500 rounded-md p-2 h-12 hover:bg-white hover:text-blue-500 hover:border-blue-600 border transition text-white btn-auth "
            >
              Ariza berish
            </button>
            {localStorage.getItem("user") ? (
              <div className="flex items-center gap-1">
                <img
                  src={`http://localhost:8080/api/file/${user.image}`}
                  alt=""
                  className="rounded-circle border-2 border-blue-800 b rounded-full me-2"
                  style={{ width: "60px", height: "60px" }}
                />
                <h1>{user.email}</h1>

                <div
                  onClick={() => logOutHandler()}
                  className="text-blue-600 cursor-pointer "
                >
                  <FontAwesomeIcon icon={faArrowRightFromBracket} />
                  Chiqish
                </div>
              </div>
            ) : (
              <button
                onClick={() => navigate("/login")}
                type="button cursor-pointer"
                className="bg-blue-500 rounded-md p-2 h-12 hover:bg-white hover:text-blue-500 hover:border-blue-600 border transition text-white btn-auth "
              >
                Kirish
              </button>
            )}
          </div>
        </div>
      </div>
      <div>
        <div className="text-center">
          <select
            className=" max-w-56 bg-transparent border outline-0 rounded-md p-2 border-none"
            onChange={(e) =>
              dispatch(setSearch({ ...search, type: e.target.value }))
            }
          >
            <option value="">Holatni tanlang</option>
            <option value="FOUND">Topilma</option>
            <option value="MISSING">Yo'qotilganlar</option>
          </select>

          <select
            className="max-w-24 bg-transparent border outline-0 rounded-md p-2 border-none"
            onChange={(e) =>
              dispatch(setSearch({ ...search, status: e.target.value }))
            }
          >
            <option value="">Ashyo turini tanlang</option>

            <option value="Buyum">Buyum</option>
            <option value="Hujjat">Hujjat</option>
          </select>

          <select
            className="max-w-56  bg-transparent border outline-0 rounded-md p-2 border-none"
            onChange={(e) =>
              dispatch(setSearch({ ...search, miniType: e.target.value }))
            }
          >
            <option value="">Ashyo sub turini tanlang</option>

            {search.status === "Buyum"
              ? item.map((item, index) => (
                  <option key={index} value={item.id}>
                    {item.name}
                  </option>
                ))
              : search.status === "Hujjat"
              ? market.map((item, index) => (
                  <option key={index} value={item.id}>
                    {item.name}
                  </option>
                ))
              : []}
          </select>

          <select
            className=" max-w-56 bg-transparent border outline-0 rounded-md p-2 border-none"
            onChange={(e) =>
              dispatch(setSearch({ ...search, region: e.target.value }))
            }
          >
            <option value="">Viloyatni tanlang</option>
            {region.map((item, index) => (
              <option key={index} value={item.id}>
                {item.name}
              </option>
            ))}
          </select>
          <select
            className=" max-w-56 bg-transparent border outline-0 rounded-md p-2 border-none"
            onChange={(e) =>
              dispatch(setSearch({ ...search, district: e.target.value }))
            }
          >
            <option value="">Viloyatni tanlang</option>
            {district.map((item, index) => (
              <option key={index} value={item.id}>
                {item.name}
              </option>
            ))}
          </select>
          <input
            className=" max-w-56 bg-transparent border outline-0 rounded-md p-2 border-none"
            type="text"
            onChange={(e) =>
              dispatch(setSearch({ ...search, serialNumber: e.target.value }))
            }
            placeholder="ABC____________"
          />
        </div>

        <div className="px-6 py-4 ">
          <table className="w-full text-sm text-left text-gray-700 border border-gray-300">
            <thead>
              <tr className="border-b">
                <td className="py-2 px-4">№</td>
                <td className="py-2 px-4">Nomi</td>
                <td className="py-2 px-4">Serial raqam</td>
                <td className="py-2 px-4">Reyestrdagi raqam</td>
                <td className="py-2 px-4">Turi</td>
                <td className="py-2 px-4">Viloyat/shahar</td>
                <td className="py-2 px-4">Tuman</td>
              </tr>
            </thead>
            <tbody>
              {reestrData.map((item, index) => (
                <tr key={index} className="border-b">
                  <td
                    onClick={() => getReestrDetails(item)}
                    className="py-2 px-4"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="20"
                      height="20"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="green"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                    >
                      <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                      <circle cx="12" cy="12" r="3"></circle>
                    </svg>
                  </td>
                  <td className="py-2 px-4">{item.entityName}</td>
                  <td className="py-2 px-4">{item.entityId}</td>
                  <td className="py-2 px-4">{item.serialNumber}</td>
                  <td className="py-2 px-4">{item.entityType}</td>
                  <td className="py-2 px-4">{item.regionName}</td>
                  <td className="py-2 px-4">{item.districtName}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </div>
    </>
  );
};

export default Reestr;
